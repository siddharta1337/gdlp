...
GoodLoop.prototype.IMG_URL = "http://goodloopcdn.blob.core.windows.net/"
...
GoodLoop.prototype.init = function(){
	...
	////// THA DOM SHIEL
			$("#domShield").on('click', function(){
				console.log("toc")
			})
			$( "div" ).on( "pageshow", function( event, ui ) {
			  var time = setTimeout(function(){
			  	$("#domShield").css("display", "none");
			  }, 200)
			});

			$( "div" ).on( "pagebeforehide", function( event, ui ) {
			  $("#domShield").css("display", "block")
			});
	
	
	///// listen when the app is back
...
GoodLoop.prototype.resumeHandler = function() {
	var currentPage = ($.mobile.activePage.attr('id'));

	switch(currentPage){
		case "myOffers": 
			app.myOffers.refreshOnResume()
			break
		case "dealList": 
			...
			break
		case "navigation": 
			app.navigationScreen.refreshList()
			break
		default:
